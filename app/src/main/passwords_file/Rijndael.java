...
public final class Rijndael {
  ...
  /**
   * AES block size in bits (N.B. the Rijndael algorithm itself allows for other
   * sizes).
   */
  ...
  private static final int[] rcon = new int[10]; /*
                                                  * for 128-bit blocks, Rijndael
                                                  * never uses more than 10 rcon
                                                  * values
                                                  */
  /**
  ...
  static {
    /*
     * Te0[x] = Se[x].[02, 01, 01, 03]; Te1[x] = Se[x].[03, 02, 01, 01]; Te2[x]
     * = Se[x].[01, 03, 02, 01]; Te3[x] = Se[x].[01, 01, 03, 02];
     * 
     * Td0[x] = Sd[x].[0e, 09, 0d, 0b]; Td1[x] = Sd[x].[0b, 0e, 09, 0d]; Td2[x]
     * = Sd[x].[0d, 0b, 0e, 09]; Td3[x] = Sd[x].[09, 0d, 0b, 0e];
     */
    int ROOT = 0x11B;
  ...
  private void invertKey() {
    ...
    for (int r = 1; r < Nr; r++) {
      ...
      rdk[d] = Td0[Se[(w >>> 24)] & 0xff] ^ Td1[Se[(w >>> 16) & 0xff] & 0xff] ^ Td2[Se[(w >>> 8) & 0xff] & 0xff]
          ^ Td3[Se[(w) & 0xff] & 0xff];
      w = rek[e + 1];
      rdk[d + 1] = Td0[Se[(w >>> 24)] & 0xff] ^ Td1[Se[(w >>> 16) & 0xff] & 0xff] ^ Td2[Se[(w >>> 8) & 0xff] & 0xff]
          ^ Td3[Se[(w) & 0xff] & 0xff];
      w = rek[e + 2];
      rdk[d + 2] = Td0[Se[(w >>> 24)] & 0xff] ^ Td1[Se[(w >>> 16) & 0xff] & 0xff] ^ Td2[Se[(w >>> 8) & 0xff] & 0xff]
          ^ Td3[Se[(w) & 0xff] & 0xff];
      w = rek[e + 3];
      rdk[d + 3] = Td0[Se[(w >>> 24)] & 0xff] ^ Td1[Se[(w >>> 16) & 0xff] & 0xff] ^ Td2[Se[(w >>> 8) & 0xff] & 0xff]
          ^ Td3[Se[(w) & 0xff] & 0xff];
      d += 4;
      e -= 4;
    }
  ...
  public void makeKey(byte[] cipherKey, int keyBits, int direction) throws RuntimeException {
    ...
    if ((direction & DIR_BOTH) != 0) {
      expandKey(cipherKey);
      /*
       * for (int r = 0; r <= Nr; r++) { System.out.print("RK" + r + "="); for
       * (int i = 0; i < 4; i++) { int w = rek[4*r + i]; System.out.print(" " +
       * Integer.toHexString(w)); } System.out.println(); }
       */
